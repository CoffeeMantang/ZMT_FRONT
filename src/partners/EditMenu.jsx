...
export default function EditMenu(props) {
  ...
  const [storeId, setStoreId] = useState(''); // response로 받아올 storeId
  ...
  const fetchMenu = async () => {
    if (props.isLogin === true) {
      ...
      if (response.data.tag !== null && response.data.tag.length > 0) {
        setTag(response.data.tag.split(','));
      }

      setImage(response.data.menuPic); // 메뉴 이미지 url
  ...
  const addMenu = async () => {
    if (props.isLogin === true) {
      ...
      const response = await axios
        .post(`http://localhost:8080/partners/store/menu/update`, fd, {
          headers: {
